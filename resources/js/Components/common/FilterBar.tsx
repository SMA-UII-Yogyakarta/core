...
function FilterSearch({ value, onChange, onSubmit, placeholder = "Cari data..." }: FilterSearchProps) {
    return (
        <form
            onSubmit={onSubmit}
    ...
    return (
        <button
            ...
            className={`px-4 py-1.5 rounded-full text-[11px] font-semibold transition-colors ${
                active ? "bg-accent text-primary" : "bg-muted text-text-muted hover:bg-border"
            }`}
            type="button"
        >
            {label}
