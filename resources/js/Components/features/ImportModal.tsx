...
export default function ImportModal({ open, onClose, entity }: ImportModalProps) {
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        if (
            dropped &&
            (dropped.name.endsWith(".xlsx") || dropped.name.endsWith(".xls") || dropped.name.endsWith(".csv"))
        ) {
            setFile(dropped);
        }
    };
            ...
            {!result ? (
                <>
                    ...
                    >
                        {file ? (
                            ...
                            <div className="flex flex-col items-center gap-2">
                                ...
                                <p className="text-[14px] text-[#64748b]">Seret file ke sini atau klik untuk memilih</p>
                                <p className="text-[12px] text-[#94a3b8]">Format: .xlsx, .xls, .csv (maks 5MB)</p>
                            </div>
                        )}
                        <input
                            ref={inputRef}
                    ...
                    <div className="mt-4 p-3 bg-[#f8fafc] rounded-lg text-[12px] text-[#64748b]">
                        ...
                        {entity === "students" ? (
                            <p>
                                nis, nisn, name/nama, class/kelas, email, phone/telepon, address/alamat,
                                birth_date/tanggal lahir, enrollment_year/tahun masuk
                            </p>
                        ) : (
                ...
                <div className="text-center py-4">
                    ...
                    {result.error_count > 0 && (
                        <p className="text-[14px] text-[#ef4444] mt-1">{result.error_count} gagal</p>
                    )}
                        ...
                        <div className="mt-4 text-left">
                            ...
                            <div className="max-h-32 overflow-y-auto bg-[#fef2f2] rounded-lg p-3">
                                {result.errors.map((err, i) => (
                                    <p key={i} className="text-[12px] text-[#dc2626] mb-1">
                                        {err}
                                    </p>
                                ))}
                            </div>
                        ...
                        <div className="mt-4 text-left">
                            ...
                            <div className="max-h-32 overflow-y-auto bg-[#f0fdf4] rounded-lg p-3">
                                {result.success.map((s, i) => (
                                    <p key={i} className="text-[12px] text-[#16a34a] mb-0.5">
                                        {s}
                                    </p>
                                ))}
                            </div>
